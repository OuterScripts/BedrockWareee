...
local function switchToAndUseTool(block, legit)
	local tool = getBestTool(block.Name)FindFirst
	if tool and (entityLibrary.isAlive and lplr.Character:Child('HandInvItem') and lplr.Character.HandInvItem.Value ~= tool.tool) then
		if legit then
...
runFunction(function()
    local resetloop = {}
    local resetloopslowdown = {value = 1}
    resetloop = GuiLibrary.ObjectsThatCanBeSaved.WorldWindow.Api.CreateOptionsButton({
        Name = 'AutoDie',
        HoverText = 'commits suicide :sob: \nbedrockware useless feature moment',
        Function = function(callback)
            if callback then
                repeat
        ...
        ExtraText = function()
            return resetloopslowdown.Value
        end
    })
...
runFunction(function()
    local lolol = {}
    lolol = GuiLibrary.ObjectsThatCanBeSaved.WorldWindow.Api.CreateOptionsButton({
